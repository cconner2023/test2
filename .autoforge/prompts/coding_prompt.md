...
- You may run `npm run build` to verify builds
- You must **NOT** commit to git
- You must **NOT** deploy
- **If your assigned feature ID is already marked as completed/passing, SKIP it immediately and move to the next feature. Do NOT re-verify or re-implement.**
- **If a feature has been deleted from the database, update accordingly and proceed**
- All algorithm/medical data comes from external documentation **VERBATIM** — NEVER modify, paraphrase, or regenerate this data
- The core navigation flow (Categories → Subcategories → Algorithm → Disposition → Write Note) must NEVER be altered
- This is a **frontend-only PWA** using **localStorage** for persistence — there is no backend server or database
...
### STEP 5.7: PAGE RELOAD PERSISTENCE TEST (MANDATORY for data features)

For any feature involving CRUD or data persistence: create unique test data (e.g., "RELOAD_TEST_12345"), verify it exists in the UI, then hard reload the page (Ctrl+Shift+R to bypass cache). After reload, verify the test data still appears. Also close and reopen the browser tab to verify persistence. If data is gone, the implementation uses in-memory storage -- run STEP 5.6 greps, find the mock pattern, and replace with real localStorage operations. Clean up test data after verification. This test catches in-memory stores like `globalThis.devStore` that pass all other tests but lose data on reload.
...
**CRITICAL: Do NOT commit to git. Do NOT deploy.** Leave code changes uncommitted. The orchestrator handles version control.
...
**Remember:** One feature per session. Zero console errors. All data from real localStorage. Leave codebase working before ending session. Do NOT commit to git or deploy.
