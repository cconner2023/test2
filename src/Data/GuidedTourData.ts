...
// ─── Map Overlay Tour Demo ──────────────────────────────────────────────────

/** Stable ID prefix so the map tour can find and clean up its mock overlay + features. */
export const MAP_TOUR_OVERLAY_PREFIX = 'tour_map_'
export const MAP_TOUR_WAYPOINT_ID = `${MAP_TOUR_OVERLAY_PREFIX}wp`
export const MAP_TOUR_ROUTE_ID = `${MAP_TOUR_OVERLAY_PREFIX}rt`

/**
 * Build a local-only mock LocalMapOverlay containing one PZ waypoint and one
 * 3-leg route. Persisted directly to IDB (no vault fan-out) and removed at
 * tour cleanup. Center sits over Fort Liberty so the demo is recognizably
 * "training area" without leaking real mission data.
 */
export function createMapTourOverlay(
  clinicId: string,
  userId: string,
): import('../Types/MapOverlayTypes').LocalMapOverlay {
  const now = new Date().toISOString()
  // Fort Liberty / Bragg approximate area — used purely for demo positioning.
  const center: [number, number] = [35.1396, -79.0061]
  const waypoint: import('../Types/MapOverlayTypes').OverlayFeature = {
    id: MAP_TOUR_WAYPOINT_ID,
    overlay_id: `${MAP_TOUR_OVERLAY_PREFIX}demo`,
    type: 'waypoint',
    geometry: [[35.1410, -79.0040]],
    label: 'CCP Bravo',
    style: { color: 'var(--color-themeblue2)', weight: 3, opacity: 1 },
    waypoint_type: 'pz',
    created_at: now,
    updated_at: now,
  }
  const route: import('../Types/MapOverlayTypes').OverlayFeature = {
    id: MAP_TOUR_ROUTE_ID,
    overlay_id: `${MAP_TOUR_OVERLAY_PREFIX}demo`,
    type: 'route',
    geometry: [
      [35.1380, -79.0090],
    ...
    label: 'Route Sapphire',
    style: { color: 'var(--color-themegreen)', weight: 3, opacity: 1 },
    created_at: now,
    updated_at: now,
  }
  return {
    id: `${MAP_TOUR_OVERLAY_PREFIX}demo`,
    clinic_id: clinicId,
    name: 'Guided Tour Demo',
    description: 'Demo overlay generated by the Map guided tour.',
    center,
    zoom: 15,
    features: [waypoint, route],
    created_by: userId,
    created_at: now,
    updated_at: now,
    _sync_status: 'synced',
