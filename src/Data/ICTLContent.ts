 ...
 * are NOT in the task PDF — they come from the skill-sheet PDF, which is authored once and
 * referenced by many tasks. A performance step that defers this way carries `skillSheetRef`
 * pointing at an entry in `ictlSkillSheets` (see bottom of file); until the skill-sheet PDF is
 * provided, that entry is a `pending` stub.
 */
...
export interface IctlSkillSheet {
  ...
  module?: string
  /** True until the skill-sheet PDF is transcribed into `steps`. */
  pending: boolean
  /** Expanded, individually gradeable steps drawn from the skill-sheet PDF (empty while pending). */
...
export const ictlTaskData: IctlTaskData[] = [
  {
    ...
    performanceSteps: [
      ...
      { number: '2a', text: 'Manage an Open Abdominal Wound.', isSubStep: true, skillSheetRef: 'jts-wound-management' },
      { number: '2b', text: 'Manage an Impaled Object.', isSubStep: true, skillSheetRef: 'jts-wound-management' },
 ...
 * Authored once from the skill-sheet PDF; a `pending` entry means the PDF hasn't been provided
 * yet, so any step referencing it currently shows the rolled-up measure only.
 */
