...
/** Shared state for closing all drawers — used when opening any single drawer or navigating from search */
...
export function useNavigation() {
    ...
    const handleNavigation = useCallback((result: SearchResultType) => {
        switch (result.type) {
            ...
            case 'training':
                // Training items are handled by App.tsx (TrainingDrawer on mobile,
                // Settings->Training on desktop). No navigation state change needed.
                break
    ...
    const setShowTrainingDrawer = useCallback((taskId: string | null) => {
        setState(prev => ({
            ...prev,
    ...
    return {
        ...
        showTrainingDrawer: state.showTrainingDrawer,
        trainingDrawerTaskId: state.trainingDrawerTaskId,
