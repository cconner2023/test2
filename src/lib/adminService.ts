...
// Build-time encrypted seed of the locations taxonomy (AES-256-GCM under the
// app-wide barcode key). Imported as a raw string (vite/client types ?raw as
// string — no resolveJsonModule needed) and parsed at runtime. Regenerated by
// scripts/build-locations-seed.mjs on prebuild; safe to commit (opaque without
// the barcode key). See the locations delta-sync block below.
import locationsSeedRaw from '../assets/locations-seed.json?raw'
...
// PostgREST egress source. Layers:
...
//     returned instantly and reconciled in the background.
//   - bundled encrypted seed (locations-seed.json): the cold-start floor on a
//     device with no persisted cache — decrypts instantly, zero network.
// Reconciliation is a DELTA: each sync asks only for rows changed since our
// high-water mark (max updated_at applied), so an idle sync returns []. This is
// the egress win — full-table reads never happen except the no-seed cold
// fallback. Works on NIPR (no storage-CDN dependency; plain PostgREST only).
let locationsCache: AdminLocation[] | null = null
let locationsHwm: string | null = null // max(updated_at) applied; the delta cursor
let locationsInflight: Promise<AdminLocation[]> | null = null

// v2: schema changed (now persists hwm). A v1 cache is silently ignored → reseed.
const LOCATIONS_LS_KEY = 'adtmc_locations_cache_v2'
...
interface PersistedLocations { ts: number; rows: AdminLocation[]; hwm: string | null }

/** Read the persisted set. Returns null on miss/corrupt; `stale` flags TTL expiry. */
function readPersistedLocations(): { rows: AdminLocation[]; hwm: string | null; stale: boolean } | null {
  try {
    const raw = localStorage.getItem(LOCATIONS_LS_KEY)
    if (!raw) return null
    const parsed = JSON.parse(raw) as PersistedLocations
    if (!parsed || !Array.isArray(parsed.rows)) return null
    return { rows: parsed.rows, hwm: parsed.hwm ?? null, stale: Date.now() - parsed.ts > LOCATIONS_TTL_MS }
  } catch {
    return null
  }
}

function writePersistedLocations(rows: AdminLocation[], hwm: string | null): void {
  try {
    localStorage.setItem(LOCATIONS_LS_KEY, JSON.stringify({ ts: Date.now(), rows, hwm }))
  } catch {
...
// ── Bundled encrypted seed + delta reconciliation ─────────────────────────
// Cold start decrypts the build-time seed (zero network). Every reconcile is a
// DELTA keyed on updated_at: only rows changed since locationsHwm come back, so
// an idle sync transfers nothing. The seed is AES-256-GCM under the app-wide
// barcode key (app_keys.barcode_v1, RLS-gated to authed users) — same model as
// profiles.avatar_blob / the voicemail greeting; non-PHI taxonomy. Editing stays
// a dev-only admin function writing straight to the `locations` table; the
// updated_at trigger stamps the change so it flows out via the same delta.

interface LocationsEnvelope { version: number; generated_at: string; key_id: string; hwm: string | null; enc: string | null }
...
/** Decrypt the build-time seed. Returns null when it's a placeholder (enc null),
 *  the barcode key is unavailable, or decryption fails — caller then cold-fetches. */
async function decryptLocationsSeed(): Promise<{ rows: AdminLocation[]; hwm: string | null } | null> {
  try {
    const env = JSON.parse(locationsSeedRaw) as LocationsEnvelope
    ...
    return Array.isArray(rows) ? { rows, hwm: env.hwm ?? null } : null
  } catch {
    return null
  }
}
 ...
 *  full live set only. When set, archived rows MUST come through so the caller
 *  can drop a freshly-archived location from its cache. */
...
/** Merge a delta into a base set keyed by id: upsert live rows, drop archived,
 *  advance the high-water mark. */
function applyLocationsDelta(
  base: AdminLocation[],
  delta: LocationDeltaRow[],
  baseHwm: string | null,
): { rows: AdminLocation[]; hwm: string | null } {
  const map = new Map(base.map((r) => [r.id, r]))
  let hwm = baseHwm
  for (const row of delta) {
    const { archived_at, updated_at, ...loc } = row
    if (archived_at) map.delete(loc.id)
    else map.set(loc.id, loc)
    if (!hwm || updated_at > hwm) hwm = updated_at
  }
  return { rows: [...map.values()], hwm }
}

/** Reconcile against the DB: establish a base (memory → persisted → seed), then
 *  apply the delta. Serves the stale base on network failure (offline-safe).
 *  Concurrent callers share one round-trip. */
function syncLocations(): Promise<AdminLocation[]> {
  if (locationsInflight) return locationsInflight
  locationsInflight = (async () => {
    try {
      let base = locationsCache
      let hwm = locationsHwm
      if (!base) {
        const persisted = readPersistedLocations()
        if (persisted) { base = persisted.rows; hwm = persisted.hwm }
      }
      if (!base) {
        const seed = await decryptLocationsSeed()
        if (seed) { base = seed.rows; hwm = seed.hwm }
      }
      // No base ⇒ no cursor either: cold fallback pulls the full live set.
      const delta = await fetchLocationsDelta(base ? hwm : null)
      const merged = applyLocationsDelta(base ?? [], delta, hwm)
      locationsCache = merged.rows
      locationsHwm = merged.hwm
      writePersistedLocations(merged.rows, merged.hwm)
      return merged.rows
    } catch (error) {
      logger.error('Failed to sync locations:', error)
      return locationsCache ?? [] // serve what we have; next call retries
    } finally {
      locationsInflight = null
    }
  })()
 ...
 *  the row just written (or the archive tombstone), so the EDITING device sees
 *  its change immediately. No snapshot rebuild — propagation rides the delta. */
async function commitLocationsMutation(): Promise<void> {
  locationsInflight = null // never reuse a sync issued before this write
  await syncLocations()
}

/**
 * List all non-archived locations. Authenticated; small set (~50–200 rows).
 * Memory → localStorage (stale-while-revalidate) → decrypted seed, with a delta
 * reconcile in the background. Concurrent callers share one round-trip.
 */
export async function listLocations(): Promise<AdminLocation[]> {
  if (locationsCache) return locationsCache
  if (locationsInflight) return locationsInflight

  // Warm cache: serve persisted instantly; delta-reconcile only past the TTL.
  const persisted = readPersistedLocations()
  if (persisted) {
    locationsCache = persisted.rows
    locationsHwm = persisted.hwm
    if (persisted.stale) void syncLocations()
    return locationsCache
  }

  // Cold device: paint from the bundled seed instantly, reconcile in background.
  const seed = await decryptLocationsSeed()
  if (seed) {
    locationsCache = seed.rows
    locationsHwm = seed.hwm
    writePersistedLocations(seed.rows, seed.hwm)
    void syncLocations()
    return locationsCache
  }

  // No usable seed (placeholder / barcode key absent) → blocking full fetch.
