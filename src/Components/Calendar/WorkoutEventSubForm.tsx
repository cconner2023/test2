...
import { logFromTemplate } from '../../lib/aft/workoutHelpers'
...
export function WorkoutEventSubForm({ workoutId, log, onWorkoutIdChange, onLogChange }: Props) {
  ...
  const handlePickWorkout = useCallback((id: string) => {
    ...
    if (!w) return
    onLogChange(logFromTemplate(w, exercises))
  }, [workouts, exercises, onWorkoutIdChange, onLogChange])
  ...
  return (
    <div className="space-y-3">
      ...
      {selected && log && log.blocks.map((logBlock, blockIdx) => {
        const isWeight = logBlock.type === 'weight'
        ...
        return (
          <div key={blockIdx} className="rounded-2xl bg-themewhite2 overflow-hidden">
            <div className="px-4 py-2.5 border-b border-primary/6 flex items-center justify-between gap-2">
      ...
      {selected && selected.exercise_ids.length === 0 && (
        <p className="px-2 text-[9pt] text-tertiary">Template has no exercises. Edit the workout template to add some.</p>
      )}
    </div>
  )
}
