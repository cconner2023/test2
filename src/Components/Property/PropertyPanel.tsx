...
export const PropertyPanel = memo(function PropertyPanel({
  ...
  // distinct token, so choosing Sign in again after cancelling the picker re-opens it.
  ...
  const handleSelectReceipt = useCallback((r: HandReceipt) => {
    ...
    closeLocationDetail()
    // A plain View drops any pending card verb, so re-opening a receipt last entered
    // via Edit or Sign in doesn't resume that verb on the fresh mount.
    setReceiptEditingId(null)
    setSignInIntent(null)
    setSelectedReceiptId(r.handReceiptId)
  }, [isMobile, view, onBack, closeLocationDetail])

  // Select FIRST in both — handleSelectReceipt clears any pending verb, so setting the
  // new one after it is what survives the batch.
  const handleEditReceipt = useCallback((r: HandReceipt) => {
    handleSelectReceipt(r)
    setReceiptEditingId(r.handReceiptId)
  }, [handleSelectReceipt])
  const handleSignInReceipt = useCallback((r: HandReceipt) => {
    handleSelectReceipt(r)
    signInSeq.current += 1
    setSignInIntent(`${r.handReceiptId}:${signInSeq.current}`)
  }, [handleSelectReceipt])
  ...
  const activeForm =
    turnInEditing && turnInZone ? (
      ...
      <Da2062Editor
        ...
        refetch={refetchReceipts}
        receipts={receipts}
        onDone={() => setReceiptEditingId(null)}
        onSavingChange={setFormSaving}
      />
  ...
  if (!isMobile) {
    ...
    return (
      <>
          ...
          <div className="flex-1 min-w-0 relative">
            ...
            {propertyTab === 'custody' && store.clinicId ? (
              <CustodyPanel
                ...
                onEditReceipt={onDeleteItem ? handleEditReceipt : undefined}
                onSignInReceipt={onDeleteItem ? handleSignInReceipt : undefined}
                onPrintReceipt={handleReprint}
                ...
                onCompleteTurnIn={onDeleteItem ? setPendingCompleteTurnIn : undefined}
              ...
              <>
                <PaneHeader
                  ...
                  actions={
                    da2062Editing ? (
                      <HeaderPill>
                        <PillButton icon={X} iconSize={16} onClick={() => setReceiptEditingId(null)} label="Cancel" />
                        <PillButton icon={Check} iconSize={16} accent="success" onClick={() => da2062EditorRef.current?.submit()} label="Save" />
                      </HeaderPill>
                    ) : (
                      <HeaderPill>
                ...
                <div className="flex-1 min-h-0 overflow-y-auto">
                  {da2062Editing ? (
                    ...
                    <Da2062Detail
                      ...
                      onEdit={() => setReceiptEditingId(selectedReceipt.handReceiptId)}
                      drawerRef={panelRef}
                      signInIntent={signInIntent?.startsWith(`${selectedReceipt.handReceiptId}:`) ? signInIntent : null}
                    />
                  )}
                </div>
              </>
              ...
              <>
                <PaneHeader
                  ...
                  eyebrow={
                    <button
                      type="button"
                      onClick={() => handleLocateSubject(selectedRecord)}
                      className="block max-w-full truncate text-left hover:text-primary active:scale-95 transition-all"
                    >
        ...
        />
        <ConfirmDialog
          visible={!!pendingCompleteTurnIn}
          title="Complete this turn-in?"
          subtitle="Marks every item as turned in to the depot."
          confirmLabel="Complete"
          variant="success"
          onConfirm={handleConfirmCompleteTurnIn}
          onCancel={() => setPendingCompleteTurnIn(null)}
        />
        <ConfirmDialog
          visible={!!pendingRemoveTurnIn}
          title="Remove this turn-in?"
          subtitle="Un-stages every item back onto the books."
          confirmLabel="Remove"
          variant="danger"
          onConfirm={handleConfirmRemoveTurnIn}
          onCancel={() => setPendingRemoveTurnIn(null)}
        />
        <ConfirmDialog
      ...
      <Sheet
        ...
        titleNode={
          ...
          selectedReceipt ? (
            ...
            // Subject above (the breadcrumb slot, as the receipt header uses it), the
            // record and its date as the title — mirroring the desktop PaneHeader. The
            // crumb navigates to the subject, matching PropertyBreadcrumb's contract.
            <div className="min-w-0">
              <button
                type="button"
                onClick={() => handleLocateSubject(selectedRecord)}
                className="block max-w-full truncate text-left text-[10pt] text-tertiary hover:text-primary active:scale-95 transition-all mb-0.5"
              >
                {selectedRecord.label}
              </button>
        ...
        leftContent={
          formDrilled ? (
            ...
            <button onClick={() => setReceiptEditingId(null)} aria-label="Cancel" className="w-9 h-9 rounded-full flex items-center justify-center text-tertiary active:scale-95 transition-all">
              <ChevronLeft size={20} />
            </button>
          ) : mobileForm ? (
