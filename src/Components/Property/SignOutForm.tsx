 ...
 * external) + 1..N items, write one shared hand receipt, then offer the 2062 PDF.
 *
 * Hosted by PropertyPanel's detail surface (right pane on desktop / detail sheet
 * on mobile — the same primitive item & zone selection use) and by the Settings
 * AccountabilityPanel's SignOutSheet wrapper. Provides its own padding so it can
 * drop straight into either scroll container. State resets on unmount, so closing
 * the host surface clears the form.
 */
export function SignOutForm({ onClose }: SignOutFormProps) {
