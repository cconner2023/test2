...
export function ProvisionalDeviceModal() {
  ...
  const { mounted, open, close } = useOverlay(deviceRole === 'provisional' && !dismissed, dismiss)

  // Tour: auto-dismiss when guided tour opens self-chat
  useEffect(() => {
    window.addEventListener('tour:messaging-dismiss-provisional', close)
    return () => window.removeEventListener('tour:messaging-dismiss-provisional', close)
  }, [close])
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-70 flex items-center justify-center pointer-events-none">
        <div
          className={`bg-themewhite rounded-2xl shadow-2xl border border-tertiary/10 mx-4 max-w-sm w-full px-6 py-5 flex flex-col gap-4 pointer-events-auto transition-all duration-300 ${open ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}
        >
