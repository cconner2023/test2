...
export function MissionEventBar({ event, isAssigned, width, left, top, height, onClick, onContextMenu }: MissionEventBarProps) {
  ...
  return (
    <button
      className={`absolute flex items-stretch rounded overflow-hidden text-left active:scale-[0.98] transition-opacity duration-100 ${
        isCompleted ? 'opacity-40' : isPressing ? 'opacity-60' : 'opacity-100'
      ...
      {...(onContextMenu ? longPressHandlers : {})}
    >
