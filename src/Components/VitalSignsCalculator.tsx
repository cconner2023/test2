...
export const VitalSignsCalculator = forwardRef<VitalSignsCalculatorHandle, VitalSignsCalculatorProps>(function VitalSignsCalculator({ value, onChange, compact = false, onRequestLmpPicker }, ref) {
    ...
    const cells: Cell[] = [
        ...
        },
        {
            ...
            input: (
                <div className={`flex items-center gap-1 ${compact ? 'w-full' : 'flex-1 justify-end'}`}>
                    <input type="text" inputMode="numeric" value={vitals.bpSys}
                        onChange={e => setValue('bpSys', e.target.value)}
                        placeholder="120"
                        className={`${compact ? compactInputCx : inputCx} ${compact ? 'flex-1' : 'w-14'}`} />
                    <span className="text-[10pt] text-tertiary">/</span>
                    <input type="text" inputMode="numeric" value={vitals.bpDia}
                        onChange={e => setValue('bpDia', e.target.value)}
                        placeholder="80"
                        className={`${compact ? compactInputCx : inputCx} ${compact ? 'flex-1' : 'w-14'}`} />
                </div>
            ),
        ...
        },
        {
            ...
            hint: tempHint || tempC
                ? { text: [tempC && `${tempC}°C`, tempHint?.label].filter(Boolean).join(' · '), color: tempHint?.color }
                : null,
            input: <input type="text" inputMode="decimal" value={vitals.temp}
                onChange={e => setValue('temp', e.target.value)}
                placeholder="98.6" className={compact ? compactInputCx : inputCx} />,
        },
        {
            key: 'ht', label: 'Ht (in)',
            hint: htCm ? { text: `${htCm}cm${htFtIn ? ` · ${htFtIn}` : ''}` } : null,
            input: <input type="text" inputMode="decimal" value={vitals.ht}
                onChange={e => setValue('ht', e.target.value)}
                placeholder="68" className={compact ? compactInputCx : inputCx} />,
        },
        {
            key: 'wt', label: 'Wt (lbs)',
            hint: wtKg ? { text: `${wtKg}kg` } : null,
            input: <input type="text" inputMode="decimal" value={vitals.wt}
                onChange={e => setValue('wt', e.target.value)}
                placeholder="170" className={compact ? compactInputCx : inputCx} />,
        },
        {
            key: 'lmp', label: 'LMP',
            hint: lmpInfo
                ...
                {cells.map(c => (
                    <div key={c.key} className={`flex flex-col gap-0.5 px-3 py-2 bg-themewhite ${c.cellClass || ''}`}>
                        <div className="flex items-baseline justify-between gap-2 min-w-0">
                            <span className="text-[9pt] font-semibold text-tertiary uppercase tracking-widest shrink-0">{c.label}</span>
                            {c.hint && (
            ...
            {cells.map(c => (
                <div key={c.key} className="border-b border-primary/6 last:border-0">
                    <div className={rowCx + ' border-0'}>
                        <span className={labelCx}>{c.label}</span>
