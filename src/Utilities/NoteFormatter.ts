...
import type { AlgorithmOptions, ScreenerConfig, decisionMakingType } from '../Types/AlgorithmTypes';
...
export function formatAlgorithmContent(
    ...
    algorithmOptions.forEach((card, index) => {
        ...
        // Screener action card — format detailed screener results
        if (card.type === 'action' && card.screenerConfig && state.screenerResponses) {
            const completedId = state.completedScreenerId || card.screenerConfig.id;
            const activeScreener: ScreenerConfig = screenerRegistry[completedId] || card.screenerConfig;
            const responses = state.screenerResponses;
            const gate = activeScreener.gate;
            const gateClosed = gate && !isScreenerGateOpen(activeScreener, responses);

            // Gate-closed = negative screen — no cognitive testing was performed
            const screenerLines: string[] = [];
            if (gateClosed) {
                screenerLines.push(`${activeScreener.title} — ${gate.negativeMessage}`);
            } else {
                const totalScore = getScreenerScore(activeScreener, responses);
                const maxScore = getScreenerMaxScore(activeScreener);
                const interp = activeScreener.interpretations.find(
                    i => totalScore >= i.minScore && totalScore <= i.maxScore,
                )?.label ?? '';
                screenerLines.push(`${activeScreener.title} (Score: ${totalScore}/${maxScore} - ${interp}):`);
            }

            activeScreener.questions.forEach((q, i) => {
                // Skip gated questions when gate was closed
                if (gateClosed && gate && i >= gate.gatedFromIndex) return;

                const val = responses[i] ?? 0;

                if (q.type === 'info') return; // skip info-only items
                ...
                if (q.type === 'check' && q.options) {
                    ...
                    if (checked.length > 0) {
                        ...
                        screenerLines.push(`  ${q.text} None`);
                    }
                    return;
                }

                // Default scale type
                const qScale = activeScreener.questions[i]?.scaleOptions ?? activeScreener.scaleOptions;
                const label = qScale.find(o => o.value === val)?.label ?? String(val);
                const scoredTag = isQuestionScored(q) ? '' : ' (not scored)';
                screenerLines.push(`  ${i + 1}. ${q.text} - [${label} (${val})]${scoredTag}`);
            });

            // Follow-up (GAD-7 difficulty question)
            if (card.screenerConfig.followUp && state.followUpResponse !== undefined && state.followUpResponse !== null) {
                const fuLabel = card.screenerConfig.followUp.options[state.followUpResponse] ?? '';
                screenerLines.push(`  Functional Difficulty: ${fuLabel}`);
            }

            sections.push(screenerLines.join('\n'));
